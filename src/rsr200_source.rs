    ...
    fn open_status(&self) -> Option<String> {
        let mut msg = String::from(
            "Reuter RSR200 support is new: verified against real hardware over both LAN \
             (wired and WiFi) and USB, on Linux/macOS and Windows alike. LAN's ÷8 decimation \
             and coarser is solid over ordinary gigabit Ethernet; ÷4/÷2 broke up even wired. \
             USB held up cleanly through ÷4, with real loss only at ÷2 (the highest rate) — \
             its own, narrower throughput ceiling. Either way, expect the link — not this \
             driver — to be what limits the \
             top end.",
        );
